...
const familyStructureOptions = [
  { id: 'living-with-parents', label: 'Living with parents', desc: 'Traditional joint setup' },
  { id: 'joint-family', label: 'Joint family', desc: 'Extended family members' },
  { id: 'independent', label: 'Independent / Nuclear', desc: 'Living alone or with spouse' },
  { id: 'hostel', label: 'Hostel / PG', desc: 'Student or working professional' },
] as const;

const dietaryOptions = [
  { id: 'strict-vegetarian', label: 'Strict Vegetarian', desc: 'No meat, eggs, or fish' },
  ...
  { id: 'halal-conscious', label: 'Halal Conscious', desc: 'Prefers halal meat' },
] as const;
...
export default function LifeArchitecturePage() {
  ...
  const [citySuggestions, setCitySuggestions] = useState<typeof indianCities>([]);
        ...
        <section className="glass-md rounded-2xl p-5 sm:p-6 border border-white/10">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              />
              {errors.pincode && (
                <p className="mt-1 text-xs text-error">{errors.pincode.message}</p>
              )}
            </div>
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder={pincodeValue?.length === 6 ? 'Select city' : 'Enter pincode first'}
                readOnly
                onFocus={() => pincodeValue?.length === 6 && setShowCityDropdown(true)}
                className={cn(
                  'w-full px-4 py-3 rounded-xl bg-white/5 border',
                  'text-midnight-50 placeholder:text-midnight-400',
                ...
                >
                  {citySuggestions.map((suggestion) => (
                    ...
                    >
                      ...
                      <span className="text-xs text-midnight-400">{suggestion.state}</span>
                    </button>
                  ))}
                </motion.div>
              )}
            ...
            <div>
                      ...
                      {relocationOptions.map((option) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <Icon className={cn('w-5 h-5', isSelected ? 'text-violet-400' : 'text-midnight-400')} />
                            <span className={cn('text-xs font-medium', isSelected ? 'text-midnight-50' : 'text-midnight-300')}>
                              {option.label}
                            </span>
                          </button>
                        );
              ...
              {errors.relocationPreference && (
                <p className="mt-1 text-xs text-error">{errors.relocationPreference.message}</p>
              )}
            </div>
          </div>
        </section>
          ...
          <div className="space-y-4">
            ...
            <div>
                      ...
                      {familyStructureOptions.map((option) => (
                        ...
                        >
                          ...
                          <div className="flex-1">
                            <p className={cn('text-sm font-medium', field.value === option.id ? 'text-midnight-50' : 'text-midnight-200')}>
                              {option.label}
                            </p>
                            <p className="text-xs text-midnight-400">{option.desc}</p>
                          </div>
                        </label>
                      ))}
                    </div>
              ...
              {errors.familyStructure && (
                <p className="mt-1 text-xs text-error">{errors.familyStructure.message}</p>
              )}
            </div>
          </div>
        </section>
        ...
        <section className="glass-md rounded-2xl p-5 sm:p-6 border border-white/10">
          ...
          <div className="space-y-4">
            ...
            <div>
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      {indianUniversities.map((uni) => (
                        <option key={uni} value={uni} className="bg-midnight-900">
                          {uni}
                        </option>
                      ))}
                    </select>
              ...
              />
              {errors.education && (
                <p className="mt-1 text-xs text-error">{errors.education.message}</p>
              )}
            </div>
            ...
            <div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {careerFields.map((career) => (
                      ...
                      >
                        ...
                        <span className={cn('text-xs font-medium text-center', field.value === career.id ? 'text-midnight-50' : 'text-midnight-300')}>
                          {career.label}
                        </span>
                      </button>
                    ))}
              ...
              />
              {errors.careerField && (
                <p className="mt-1 text-xs text-error">{errors.careerField.message}</p>
              )}
            </div>
          </div>
        </section>
        ...
        <section className="glass-md rounded-2xl p-5 sm:p-6 border border-white/10">
          ...
          <div>
                  ...
                  {dietaryOptions.map((option) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <p className={cn('text-sm font-medium', isSelected ? 'text-midnight-50' : 'text-midnight-200')}>
                            {option.label}
                          </p>
                          <p className="text-xs text-midnight-400">{option.desc}</p>
                        </div>
            ...
            {errors.dietaryPreferences && (
              <p className="mt-1 text-xs text-error">{errors.dietaryPreferences.message}</p>
            )}
          </div>
        </section>
