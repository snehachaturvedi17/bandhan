...
const loveLanguages: LoveLanguage[] = [
  {
    ...
    description: 'Showing love through respect and care for parents/grandparents',
    icon: HandHeart,
  },
  {
    id: 'quality-time',
...
const dealbreakers: Dealbreaker[] = [
  {
    ...
    label: 'Smoking',
    labelHi: 'धूम्रपान',
    levels: ['Dealbreaker', 'Dislike but acceptable', "Don't mind", 'I do it too'],
  },
  {
    id: 'drinking',
    label: 'Drinking alcohol',
    labelHi: 'शराब पीना',
    levels: ['Dealbreaker', 'Occasional is fine', "Don't mind", 'I enjoy it too'],
  },
  {
    id: 'late-night',
    label: 'Late-night outings',
    labelHi: 'देर रात बाहर जाना',
    levels: ['Never acceptable', 'With friends/family', 'Occasionally', 'I do it too'],
  },
  {
    id: 'social-media',
    label: 'Social media usage',
    labelHi: 'सोशल मीडिया उपयोग',
    levels: ['Minimal preferred', 'Moderate is fine', "Don't mind", 'Very active myself'],
  },
];
...
export default function ValuesPersonalityPage() {
  ...
  const [dealbreakerLevels, setDealbreakerLevels] = useState<Record<string, number>>({});
      ...
      case 'dealbreakers':
        return (
          <div className="space-y-6">
                ...
                >
                  ...
                  <div className="space-y-2">
                    {breaker.levels.map((level, levelIndex) => {
                      const isSelected = dealbreakerLevels[breaker.id] === levelIndex;
                      return (
                        <button
                          key={level}
                          type="button"
                          onClick={() => setDealbreakerLevel(breaker.id, levelIndex)}
                          className={cn(
            ...
            <p className="text-xs text-center text-midnight-500">
              Answered: {Object.keys(dealbreakerLevels).length} of {dealbreakers.length}
            </p>
          </div>
        );
